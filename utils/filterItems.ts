    ...
    if (!emptyInput) {
      ...
      if (label === filterText) {
        ...
        const idx = label.indexOf(
          filterText.slice(0, (positions[positions.length - 1] ?? 0) + 1),
        );
        if (idx !== -1) {
    ...
    if (originItem.textEdit && InsertReplaceEdit.is(originItem.textEdit)) {
      return {
        ...
        textEdit: TextEdit.replace(
          originItem.textEdit.replace,
          originItem.textEdit.newText,
        ),
      };
    }
    return originItem;
...
function nextScore(
  codes: ReadonlyArray<number>,
  index: number,
  inputCodes: ReadonlyArray<number>,
  positions: ReadonlyArray<number>,
): [number, ReadonlyArray<number>] | undefined {
