import { type PublishDiagnosticsParams, type Diagnostic } from "vscode-languageserver-protocol";
...
class DiagnosticCenter {
  ...
  getDiagnosticsByFilePath(filePath: string): Diagnostic[] {
    ...
    return itemsOfProject?.filter(item => item.uri === uri).reduce((prev, cur) => {
      return prev.concat(cur.diagnostics);
    }, [] as Diagnostic[]) ?? []
  }

  setDiagnosticsByProjectRoot(projectRoot: string, source: string, diagnosticsParams: PublishDiagnosticsParams) {
    let items = this.diagnosticMap.get(projectRoot);
    if (!items) {
      items = [];
      this.diagnosticMap.set(projectRoot, items);
    }
    const theItem = items.find(item => item.uri === diagnosticsParams.uri && item.source === source);
    if (!theItem) {
      items.push({
        ...diagnosticsParams,
        source,
