...
export function convert_emacs_bool(
  symbol_value: any,
  symbol_is_boolean: 't' | 'nil',
) {
...
export async function get_emacs_vars(args: string[]) {
  const results = await epc_client?.callMethod<Array<[any, 't' | 'nil']>>(
    'get-emacs-vars',
    args,
  );
...
export async function get_emacs_var(var_name: string) {
  const result = await epc_client?.callMethod<[any, 't' | 'nil']>(
    'get-emacs-var',
    var_name,
  );
...
export async function get_emacs_func_result<T>(
  method_name: string,
  ...args: any[]
): Promise<T> {
  const _args = [symbol(method_name), ...args.map(handle_arg_types)];
  const sexp = encode(_args);
  const result = await epc_client?.callMethod<any>('get-emacs-func-result', [
    sexp,
  ]);
